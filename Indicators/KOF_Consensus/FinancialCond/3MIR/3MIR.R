...
ts_df(ts_c(
  ...
  ggplot() +
  geom_ribbon(mapping = aes(x = time, ymin = kof_cons_3minterest_3m.min, ymax = kof_cons_3minterest_3m.max), fill = "lightgray", alpha = 0.5) +
  geom_ribbon(mapping = aes(x = time, ymin = kof_cons_3minterest_3m.mean - kof_cons_3minterest_3m.stdev, ymax = kof_cons_3minterest_3m.mean + kof_cons_3minterest_3m.stdev), fill = "darkgrey", alpha = 0.5) +
  geom_line(mapping = aes(x = time, y = kof_cons_3minterest_3m.mean), color = "black", linewidth = 1) +
  geom_hline(yintercept = 0, color = "black", linetype = "dashed", show.legend = FALSE) +
  scale_x_date(limits = c(date(start_date), today()), date_breaks = "1 year", date_labels = "%Y") +
  scale_y_continuous(limits = c(-1, 4)) +
  theme_bw() +
  labs(
    title = "KOF consensus 3-month interest rate forecast - 3 months ahead",
    subtitle = "",
    caption = "Graph created by @econmaett with data from KOF.",
    x = "", y = ""
  )

ggsave(filename = "Indicators/KOF_Consensus/FinancialCond/3MIR/KOF_Cons3MIR_3m.png", width = 8, height = 4)
graphics.off()
...
ts_df(ts_c(
  ...
  pivot_wider(names_from = id, values_from = value) |> 
  ggplot() +
  geom_ribbon(mapping = aes(x = time, ymin = kof_cons_3minterest_12m.min, ymax = kof_cons_3minterest_12m.max), fill = "lightgray", alpha = 0.5) +
  geom_ribbon(mapping = aes(x = time, ymin = kof_cons_3minterest_12m.mean - kof_cons_3minterest_12m.stdev, ymax = kof_cons_3minterest_12m.mean + kof_cons_3minterest_12m.stdev), fill = "darkgrey", alpha = 0.5) +
  geom_line(mapping = aes(x = time, y = kof_cons_3minterest_12m.mean), color = "black", linewidth = 1) +
  geom_hline(yintercept = 0, color = "black", linetype = "dashed", show.legend = FALSE) +
  scale_x_date(limits = c(date(start_date), today()), date_breaks = "1 year", date_labels = "%Y") +
  scale_y_continuous(limits = c(-1, 4)) +
  theme_bw() +
...
ggsave(filename = "Indicators/KOF_Consensus/FinancialCond/3MIR/KOF_Cons3MIR_12m.png", width = 8, height = 4)
graphics.off()
