...
ts_df(ts_c(kof_cons_pr_cy.max, kof_cons_pr_cy.mean, kof_cons_pr_cy.min, kof_cons_pr_cy.stdev)) |> 
  ...
  theme_bw() +
  labs(
    title = "KOF consensus inflation forecast - current year",
    subtitle = "",
    caption = "Graph created by @econmaett with data from KOF.",
    x = "", y = ""
  )

ggsave(filename = "Indicators/KOF_Consensus/Prices/KOF_ConsPrices_cy.png", width = 8, height = 4)
graphics.off()
...
ts_df(
  ...
  theme_bw() +
  labs(
    title = "KOF consensus inflation forecast - next year",
    subtitle = "",
    caption = "Graph created by @econmaett with data from KOF.",
    x = "", y = ""
  )

ggsave(filename = "Indicators/KOF_Consensus/Prices/KOF_ConsPrices_ny.png", width = 8, height = 4)
graphics.off()
...
ts_df(ts_c(kof_cons_pr_5y.max, kof_cons_pr_5y.mean, kof_cons_pr_5y.min, kof_cons_pr_5y.stdev)) |> 
  ...
  theme_bw() +
  labs(
    title = "KOF consensus inflation forecast - 5 years",
    subtitle = "",
    caption = "Graph created by @econmaett with data from KOF.",
    x = "", y = ""
  )
