...
ts_df(ts_c(
  ...
  ggplot() +
  geom_ribbon(mapping = aes(x = time, ymin = kof_cons_usdchf_3m.min, ymax = kof_cons_usdchf_3m.max), fill = "lightgray", alpha = 0.5) +
  geom_ribbon(mapping = aes(x = time, ymin = kof_cons_usdchf_3m.mean - kof_cons_usdchf_3m.stdev, ymax = kof_cons_usdchf_3m.mean + kof_cons_usdchf_3m.stdev), fill = "darkgrey", alpha = 0.5) +
  geom_line(mapping = aes(x = time, y = kof_cons_usdchf_3m.mean), color = "black", linewidth = 1) +
...
ggsave(filename = "Indicators/KOF_Consensus/ExchangeRates/USDCHF/KOF_ConsUSDCHF_3m.png", width = 8, height = 4)
graphics.off()
...
ts_df(ts_c(
  ...
  pivot_wider(names_from = id, values_from = value) |> 
  ggplot() +
  geom_ribbon(mapping = aes(x = time, ymin = kof_cons_usdchf_12m.min, ymax = kof_cons_usdchf_12m.max), fill = "lightgray", alpha = 0.5) +
  geom_ribbon(mapping = aes(x = time, ymin = kof_cons_usdchf_12m.mean - kof_cons_usdchf_12m.stdev, ymax = kof_cons_usdchf_12m.mean + kof_cons_usdchf_12m.stdev), fill = "darkgrey", alpha = 0.5) +
  geom_line(mapping = aes(x = time, y = kof_cons_usdchf_12m.mean), color = "black", linewidth = 1) +
  geom_hline(yintercept = 1, color = "black", linetype = "dashed", show.legend = FALSE) +
  scale_x_date(limits = c(date(start_date), today()), date_breaks = "1 year", date_labels = "%Y") +
  scale_y_continuous(limits = c(0.5, 2)) +
  theme_bw() +
...
ggsave(filename = "Indicators/KOF_Consensus/ExchangeRates/USDCHF/KOF_ConsUSDCHF_12m.png", width = 8, height = 4)
graphics.off()
